...

{
	...
	const int M = 3; // Window Size
	const int m = (M - 1) / 2; // Padding parameter
	const int ext = 2 * m;//Number of extra rows and columns to be added
	unsigned char Imagedata_extend[height + ext][width + ext] = { 0 };

	for (i = m; i < height + m; i++)
	{
		for (j = m; j < width + m; j++)
		{
			Imagedata_extend[i][j] = Imagedata_final[i - m][j - m];
		}
	}
	...
	for (i = 0; i < m; i++)
	{
		for (j = m; j < width + m; j++)
		{
			Imagedata_extend[i][j] = Imagedata_extend[m][j];
		}
	}
	...
	for (i = height + m; i < height + ext; i++)
	{
		for (j = m; j < width + m; j++)
		{
			Imagedata_extend[i][j] = Imagedata_extend[height + m - 1][j];
		}
	}
	...
	for (i = 0; i < height + ext; i++)
	{
		for (j = 0; j < m; j++)
		{
			Imagedata_extend[i][j] = Imagedata_extend[i][m];
		}
	}
	...
	for (i = 0; i < height + ext; i++)
	{
		...
		{
			Imagedata_extend[i][j] = Imagedata_extend[i][width + m - 1];
		}
	}


	unsigned char Imagedata_Extend_Final[height][width] = { 0 };
		...
		for (a = m; a < height + m; a++)
		{
			for (b = m; b < width + m; b++)
			{
				if ((int)Imagedata_extend[a][b] == 128)
				{
					bool flag = false;
					for (c = a - m; c <= a + m; c++)
					{
						for (d = b - m; d <= b + m; d++)
						{
							if ((int)Imagedata_extend[c][d] == 64 || (int)Imagedata_extend[c][d] == 192)
							{
								Imagedata_Extend_Final[a-m][b-m] = (unsigned char)255;
								flag = true;
							}
					...
					if (flag == false)
					{
						Imagedata_Extend_Final[a - m][b - m] = (unsigned char)0;
					}
	...
	fwrite(Imagedata_Extend_Final, sizeof(unsigned char), height*width * 1, file);
	fclose(file);
	/*

...

{
	...
	for (i = 0; i < N; i++)
	{
		for (j = 0; j < N; j++)
		{
			Gx_conv_1d[i*N + j] = Gx_conv[i][j];
			Gy_conv_1d[i*N + j] = Gy_conv[i][j];
		}
	}


	int** Imagedata_Gx = convolver(Imagedata_Pad, Gx_conv_1d, n, N, height, width);
	int** Imagedata_Gy = convolver(Imagedata_Pad, Gy_conv_1d, n, N, height, width);
	...
	for (i = 0; i < height; i++)
	{
		for (j = 0; j< width; j++)
		{
			Img_1D[i*width + j] = Imagedata_G[i][j];
		}
	}
	int count = 0;
	...
	fwrite(Imagedata_xgrad, sizeof(unsigned char), height*width * 1, file);
	fclose(file);

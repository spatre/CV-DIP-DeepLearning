...

{
	...
	unsigned char Imagedata_M[Size1][Size2];
...
// To separate the channels from 3D array to 1D array as R,G,B
		for(i=0;i<Size1;i++)
		{
			for(j=0;j<Size2;j++)
			{
				...
				Imagedata_g[i*Size2+j]=(int)Imagedata[i][j][1] ;
				//Imagedata_b[i*Size2+j]=(int)Imagedata[i][j][2] ;
		...
		for(i=0;i<Size1;i++)
		{
			for(j=0;j<Size2;j++)
			{
				 Imagedata_M[i][j]=(1.0f-Imagedata_g[i*Size2+j]/255.0f)*255.0f;
	...
	fwrite(Imagedata_M, sizeof(unsigned char), Size1*Size2, file);
	fclose(file);

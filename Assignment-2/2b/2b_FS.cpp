...

{
	...
	// 3- FS matrix, 5- JJN and Stucki matrix.
	int i, j, a, b, c, d;
	...
	for (i = 0; i < height; i++)
		{
			for (j = 0; j < width; j++)
			{
				...
				if (j + 1<width)
				{
					Imagedata_tmp[i][j + 1][0] = Imagedata_tmp[i][j + 1][0] + error * (mat_diffused[1][2]) / div;
				}
				if (j - 1 >= 0 && i + 1<height)
				{
					Imagedata_tmp[i + 1][j - 1][0] = Imagedata_tmp[i + 1][j - 1][0] + error * (mat_diffused[2][0]) / div;
				}
				if (i + 1<height)
				{
					Imagedata_tmp[i + 1][j][0] = Imagedata_tmp[i + 1][j][0] + error * (mat_diffused[2][1]) / div;
				}
				if (i + 1<height && j + 1<width)
				{
					Imagedata_tmp[i + 1][j + 1][0] = Imagedata_tmp[i + 1][j + 1][0] + error * (mat_diffused[2][2]) / div;
				}
